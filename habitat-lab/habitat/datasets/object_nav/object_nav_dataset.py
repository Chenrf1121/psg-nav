...
class ObjectNavDatasetV1(PointNavDatasetV1):
    ...
    def from_json(
        ...
        elif "episodes" in deserialized and len(deserialized["episodes"]) == 0:
            # OVON-style top-level manifest may omit content_scenes_path and keep
            # all real episodes in content/<scene>.json.gz shards.
        ...
        if "category_to_task_category_id" in deserialized:
            # OVON shards may include this field as an empty dict. Merge
            # non-empty mappings instead of clobbering categories loaded from
            # previous shards.
            incoming_task_categories = deserialized[
                "category_to_task_category_id"
            ]
            if len(incoming_task_categories) > 0:
                self.category_to_task_category_id.update(
                    incoming_task_categories
                )

        if "category_to_scene_annotation_category_id" in deserialized:
            incoming_scene_categories = deserialized[
                "category_to_scene_annotation_category_id"
            ]
            if len(incoming_scene_categories) > 0:
                self.category_to_scene_annotation_category_id.update(
                    incoming_scene_categories
                )
        elif "category_to_mp3d_category_id" in deserialized:
            incoming_scene_categories = deserialized[
        ...
        if len(self.category_to_task_category_id) > 0 or len(self.category_to_scene_annotation_category_id) > 0:
        ...
        episode_categories = sorted(
            {
                ...
                and episode["object_category"] is not None
            }
        )
        if episode_categories:
            if len(self.category_to_task_category_id) == 0:
                self.category_to_task_category_id = {
                    category_name: category_index
                    for category_index, category_name in enumerate(episode_categories)
                }
            else:
                next_category_index = max(self.category_to_task_category_id.values(), default=-1) + 1
                for category_name in episode_categories:
                    if category_name not in self.category_to_task_category_id:
                        self.category_to_task_category_id[category_name] = next_category_index
                        next_category_index += 1

            if len(self.category_to_scene_annotation_category_id) == 0:
                self.category_to_scene_annotation_category_id = {
                    category_name: category_index
                    for category_index, category_name in enumerate(episode_categories)
                }
            else:
                next_category_index = max(
                    self.category_to_scene_annotation_category_id.values(), default=-1
                ) + 1
                for category_name in episode_categories:
                    if category_name not in self.category_to_scene_annotation_category_id:
                        self.category_to_scene_annotation_category_id[category_name] = next_category_index
                        next_category_index += 1
        ...
        for i, episode in enumerate(deserialized["episodes"]):
            if "children_object_categories" in episode:
                ...
                episode.pop("children_object_categories", None)
