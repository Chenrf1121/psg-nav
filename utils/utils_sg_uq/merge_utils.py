...
import faiss
from .graph_utils import MapObjectList, DetectionList
from .iou_utils import compute_3d_iou_accuracte_batch, compute_iou_batch, mask_subtract_contained
...
def filter_gobs(
    ...
    for k in gobs.keys():
        if k == "image_rgb":  # added by yinhang
            continue
...
def create_object_pcd(depth_array, mask, cam_K, image, obj_color=None, is_navigation=False) -> o3d.geometry.PointCloud:
    if is_navigation == False:  # added by yinhang
...
def merge_obj2_into_obj1(cfg, obj1, obj2, run_dbscan=True):
    ...
    for k in obj1.keys():
        ...
        elif k in ['captions_sorted', 'caption_probs', 'caption_counts', 'caption_entropy']:
            pass
