...
class FMMPlanner():
    ...
    def set_multi_goal(self, goal_map, state):
        ...
        # Handle both single and multiple goal points
        # Check which goal points are traversible
        traversible_mask = self.traversible[goal_x, goal_y] > 0.

        if np.any(~traversible_mask):
            # Some goal points are not traversible
            if np.any(traversible_mask):
                # Keep only traversible goal points
                goal_x = goal_x[traversible_mask]
                goal_y = goal_y[traversible_mask]
            else:
                # All goal points are not traversible, find nearest traversible point
                # Use the center of the goal region as reference
                center_x = int(np.mean(goal_x))
                center_y = int(np.mean(goal_y))
                center_goal = self._find_nearest_goal([center_x, center_y], state)
                # Replace all goal points with the nearest traversible center
                goal_x = np.array([center_goal[0]])
                goal_y = np.array([center_goal[1]])
