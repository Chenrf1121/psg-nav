...
class Semantic_Mapping(nn.Module):
    ...
    def __init__(self, agent, max_height=None, min_height=None, num_cats=1):
        ...
        self.du_scale = 1
        self.min_depth_cm = float(getattr(agent.config.SIMULATOR.DEPTH_SENSOR, 'MIN_DEPTH', 0.1) * 100.0)
        self.max_depth_cm = float(getattr(agent.config.SIMULATOR.DEPTH_SENSOR, 'MAX_DEPTH', 10.0) * 100.0)
        self.exp_pred_threshold = 10
        self.map_pred_threshold = 10
        self.occupancy_erosion_kernel = 3
    ...
    def forward(self, depth, pose_obs, maps_last, type_mask=None, type_prob=None):
        ...
        valid_depth = torch.isfinite(depth) & (depth >= self.min_depth_cm) & (depth <= self.max_depth_cm)
        ...
        feat = valid_depth[:, ::self.du_scale, ::self.du_scale].reshape(1, 1, -1).float().to(self.device)
        ...
        translated = (translated > 0.5).float()
        if self.occupancy_erosion_kernel >= 3:
            k = self.occupancy_erosion_kernel
            pad = k // 2
            translated = 1.0 - F.max_pool2d(1.0 - translated, kernel_size=k, stride=1, padding=pad)
    ...
    def forward_(self, depth, pose_obs, maps_last, type_mask=None, type_prob=None):
        ...
        valid_mask = valid_depth.unsqueeze(1).float()
        self.feat = nn.AvgPool2d(self.du_scale)((type_mask * valid_mask)[:,:,:,:]
        ...
        # NOTE: Commented out to disable semantic color assignment in occupancy map
